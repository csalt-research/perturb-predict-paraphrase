...
def train(opt):
    ...
    unlab_loader = BuDataLoader(opt, False, unlab_split, opt.labelfile)
...
#             labeled_loss = 0
            undata = unlab_loader.get_batch('train')
            tmp = [undata['fc_feats'], undata['att_feats'], undata['labels'], undata['att_masks']]
            tmp = [i if i is None else i.cuda() for i in tmp]
            fc_feats, att_feats, labels, att_masks = tmp
            del tmp
            labels = labels.long()
            aug_unlab_logprobs = model(fc_feats, att_feats, labels, att_masks)
            unloss = criterion_label(aug_unlab_logprobs, labels[:, 1:]).mean()
            unlabeled_loss = unloss / acc_steps
            train_loss = labeled_loss + unlabeled_loss
            train_loss.backward()
            ...
            if not sc_flag:
                print("iter {} (epoch {}), label_loss = {:.3f}, unlabel_loss = {:.3f}" \
                .format(iteration, epoch, labeled_loss,unlabeled_loss), flush=True)
