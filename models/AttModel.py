...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        if seq is not None:
            ...
            for i in range(seq.size(1) - 1):
                if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
                    ...
                    if sample_mask.sum() == 0:
                        ...
                        it.index_copy_(0, sample_ind, torch.multinomial(prob_prev[sample_ind], 1).view(-1))
                else:
                    it = seq[:, i].clone()          
            ...
            # differentiable sequence generation step using greedy decoding
            # above code re-used with minor modifications
            ...
            it = fc_feats.new_zeros(batch_size, dtype=torch.long) # input <bos>
            output, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            outputs[:, 0] = output

            for i in range(1, self.seq_length):
                prob_prev = torch.exp(outputs[:, i-1].detach()) # fetch prev distribution: shape Nx(M+1)
                _, it = torch.max(prob_prev, 1)
                output, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
                outputs[:, i] = output
    ...
    def _diverse_sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        diversity_lambda = opt.get('diversity_lambda', 1.5)
