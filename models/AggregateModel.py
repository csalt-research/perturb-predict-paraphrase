...
class Encoder(nn.Module):
	def __init__(self, embedding_layer, emb_dim, hid_dim, n_layers, dropout, n_aug, bi):
		...
		self.rnn = nn.LSTM(emb_dim*self.n_aug, hid_dim, n_layers, dropout = dropout, bidirectional=bi)
		
	def forward(self, src, src_len,mask):
...
class Seq2Seq(nn.Module):
	...
	def forward(self, src):
		...
		hidden, cell = self.encoder(src, src_len, mask.float())
